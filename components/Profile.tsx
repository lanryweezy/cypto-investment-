...
import { User, Wallet, LogOut, Settings, CreditCard, History, TrendingUp, PieChart, ShieldCheck, Briefcase, Star, Download, Zap } from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ user, setUser, currentPrices, onToggleWatchlist }) => {
  ...
  const [tempBalance, setTempBalance] = useState(user?.balance || 0);
  ...
  useEffect(() => {
    const loadPlan = async () => {
      if (user) {
        const plan = await paymentService.getActivePlanForUser(user.id);
        setCurrentPlan(plan);
      }
    };
  ...
  const handleSaveBalance = async () => {
    ...
    const updatedUser = { ...user, balance: tempBalance };
    setUser(updatedUser);
    
    try {
      await databaseService.updateUserBalance(user.id, tempBalance);
      ...
      <div className="glass-panel p-8 rounded-2xl border border-crypto-accent/20 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="flex gap-3 relative z-10">
            ...
            {currentPlan && currentPlan.id !== 'free' && (
              <div className="flex items-center gap-2 px-4 py-2 bg-crypto-accent/10 rounded-lg border border-crypto-accent/20">
                <Zap size={16} className="text-crypto-accent" />
                <span className="text-sm font-bold text-crypto-accent">{currentPlan.name} Plan</span>
              </div>
            )}
        </div>
      </div>
            ...
            <div className="glass-panel p-6 rounded-2xl">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-crypto-accent hover:text-crypto-secondary transition-colors"
                  >
                    <Zap size={12} />
                    Upgrade
                  </button>
